...
The forced-collection storm suites, `BridgeSpec` and `CurlBufferSpec`, call `System.gc()` back to back, which keeps commix's heap-growth rule firing: with the default `GC_TIME_RATIO` of 0.05, every test process grew its heap to tens of gigabytes, or to a CI runner's whole memory, where a large allocation then exited the process with `Out of heap space` (#65). `build.sbt` therefore runs the `providers` and `app` test processes with `GC_TIME_RATIO=0.9`, unless the variable is already set in the environment of the process that starts the sbt server. The app itself keeps the commix default.
