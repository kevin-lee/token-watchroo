...
The forced-collection storm suites, `BridgeSpec` and `CurlBufferSpec`, call `System.gc()` back to back, which keeps commix's heap-growth rules firing: with the defaults every test process grew its heap to the machine's whole memory, where an allocation that missed its retries exited the process with `Out of heap space` (#65). `build.sbt` therefore runs the `providers` and `app` test processes with `GC_TIME_RATIO=1.0` and `GC_FREE_RATIO=0`, which switch those two rules off, and with `GC_MAXIMUM_HEAP_SIZE=2G`. `TW_STORM_GC_DEFAULTS=1` in the environment of the process that starts the sbt server keeps the commix defaults instead.
